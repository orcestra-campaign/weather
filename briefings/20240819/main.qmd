---
title: "PERCUSION Weather briefing"
---

# Current synoptic situation
## NHC surface analysis
![]({{< meta plots.external_inst.nhc_surface_analysis_atlantic >}})

## NHC tropical weather outlook
![]({{< meta plots.external_inst.nhc_seven_days_outlook >}})

## 
:::: {.columns}
::: {.column width="7%"}
NHC tropical Hovmöller
:::
::: {.column width="93%"}
![]({{< meta plots.external_inst.nhc_hovmoller >}}){width="60%"}
:::
::::


# Latest satellite images
## GOES yesterday's image
![]({{< meta plots.external_inst.yesterdays_goes2go_image >}})

## GOES latest image
![]({{< meta plots.external_inst.latest_goes2go_image >}})

## GOES WEST Latest Infrared Image
![]({{< meta plots.external_inst.current_satellite_image_ir >}})

# ECMWF Forecasts

## 
### {{< meta valid_dates.012h >}}
::: {layout-nrow=2}
![]({{< meta plots.internal.iwv_itcz_edges.012h >}}){width=42%}
![]({{< meta plots.internal.sfc_winds.012h >}}){width=42%}
![]({{< meta plots.internal.cloud_top_height.012h >}}){width=42%}
![]({{< meta plots.internal.precip.012h >}}){width=42%}
:::

## 
### {{< meta valid_dates.036h >}}
::: {layout-nrow=2}
![]({{< meta plots.internal.iwv_itcz_edges.036h >}}){width=42%}
![]({{< meta plots.internal.sfc_winds.036h >}}){width=42%}
![]({{< meta plots.internal.cloud_top_height.036h >}}){width=42%}
![]({{< meta plots.internal.precip.036h >}}){width=42%}
:::

## 
### {{< meta valid_dates.060h >}}
::: {layout-nrow=2}
![]({{< meta plots.internal.iwv_itcz_edges.060h >}}){width=42%}
![]({{< meta plots.internal.sfc_winds.060h >}}){width=42%}
![]({{< meta plots.internal.cloud_top_height.060h >}}){width=42%}
![]({{< meta plots.internal.precip.060h >}}){width=42%}
:::

## 
### {{< meta valid_dates.084h >}}
::: {layout-nrow=2}
![]({{< meta plots.internal.iwv_itcz_edges.084h >}}){width=42%}
![]({{< meta plots.internal.sfc_winds.084h >}}){width=42%}
![]({{< meta plots.internal.cloud_top_height.084h >}}){width=42%}
![]({{< meta plots.internal.precip.084h >}}){width=42%}
:::

## 
### {{< meta valid_dates.108h >}}
::: {layout-nrow=2}
![]({{< meta plots.internal.iwv_itcz_edges.108h >}}){width=42%}
![]({{< meta plots.internal.sfc_winds.108h >}}){width=42%}
![]({{< meta plots.internal.cloud_top_height.108h >}}){width=42%}
![]({{< meta plots.internal.precip.108h >}}){width=42%}
:::

# Saharan dust forecast
##
:::: {.columns}
::: {.column  width="93%"}
::: {layout-nrow=2}
![{{< meta valid_dates.012h >}}]({{< meta plots.external_lead.dust.012h >}}){width=75%}

![{{< meta valid_dates.036h >}}]({{< meta plots.external_lead.dust.036h >}}){width=75%}

![{{< meta valid_dates.060h >}}]({{< meta plots.external_lead.dust.060h >}}){width=75%}

![{{< meta valid_dates.084h >}}]({{< meta plots.external_lead.dust.084h >}}){width=75%}
:::
:::
::: {.column width="7%"}
![](dust_colorbar.png){width=100%}
:::
::::

##
:::: {.columns}
::: {.column width="30%"}
### Local forecast for {{< meta location >}}
:::
::: {.column width="70%"}
![]({{< meta plots.external_inst.ifs_meteogram >}}){width="61%"}
:::
::::

## Summary
- For the next two days (ATR flights):
    - low wind region with deep convection to the south of Sal
    - ITCZ still quite far north, with the moist air tongue from the previous wave moving westward
- For the upcoming flights on Wednesday:
    - circles might have to be adapted, northern moist margin relatively certain, CTH ~11-12km
- For the upcoming flights on Thursday:
    - Flight planning uncertain, northern moist margin north of Sal, very broad moist region